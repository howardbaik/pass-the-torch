...
Also initializes layer norms, attention module drop path, mlp, projection layer, and skip connection pooling.

#### `forward()` method 

Applies LayerNorm, then pooling attention with `MultiScaleAttention`. Then applies another LayerNorm and an MLP to get the output.





**File: `mvit/models/mvit_model.py`**

1. `PatchEmbed` class: 

Patch embedding layer that converts raw input images into sequence of token embeddings for transformer. Uses a 2D convolution (`nn.Conv2d`)
...
#### Forward method:
- 2D convolution converts input `x`: `(B, 3, H, W) → (B, 768, H', W')`. It projects each path into a high dimensional embedding space (768). Using `Conv2d` is more efficient than the ViT approach (linear layer on flattened patches).
...
Pulls settings from the config: 
- input size
- number of classes
- embedding dimension
- number of heads
...
The forward pass of MViT transforms a raw input image into class predictions through a sequence of operations:
