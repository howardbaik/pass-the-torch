...
This post is a code-focused deep dive into a echocardiographic deep learning model called PanEcho. PanEcho is a multivew, multitask, video-based deep learning model capable of performing the full range of key echocardiographic reporting tasks.
...
(From the PanEcho Implementation Details in eMethods)

Steps
1. Each video frame is first processed by the 2D image encoder, an ImageNet5-pretrained ConvNeXt-T3 convolutional neural network (CNN) which produces a learned feature vector, or representation, of each frame. 

2. These frame-wise representations are then interpreted as an ordered sequence – like words in a sentence in natural language processing and modeled using self-attention to learn time-varying associations over the frames. 

3. Frame order is embedded via sinusoidal positional encoding, which is then elementwise added to the frame-wise feature vectors and fed to a Transformer encoder consisting of four layers, each with eight self-attention heads. 

4. Mean pooling is then used to aggregate frame-wise feature vectors into a single video-level representation, which is used as input to the task-specific output heads. Each output head consists of a Dropout layer with probability 0.25 and a fully-connected layer. 
