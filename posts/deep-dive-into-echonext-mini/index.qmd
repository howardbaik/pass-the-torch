...
The model architecture is a 1-D adaptation of ResNet-34 taking a 12-lead ECG as input with the final layer modified to accept tabular features alongside the learned ECG features. The final layer outputs 12 logits, with 1 detecting composite SHD and the other 11 detecting individual structural heart disease (SHD) subdiagnoses. The model was trained on the [EchoNext dataset](https://physionet.org/content/echonext/1.1.1/) and outputs a probability between 0 and 1. 

In the rest of this post, we will go through the EchoNext-Mini [source code](https://github.com/howardbaik/IntroECG/tree/master/7-EchoNext%20Minimodel), which is implemented in PyTorch, and explain how the model works. The code is organized into two main classes: `BasicBlock1d` and `ResNet1dWithTabular`.



## The `BasicBlock1d` class

The `BasicBlock1d` class implements the pieces of a residual block that make up the ResNet-34 architecture. 

### The `__init__()` method

The `BasicBlock1d` class is first initialized with `nn.Conv1d`, which is PyTorch's 1D convolution layer - it slides learnable filters along a single dimension (here, time) instead of over a 2D image. It slides the filters along time with the parameters `stride`, `padding`, and `kernel_size`. Also, it has `nn.BatchNorm1d` for batch normalization, `nn.ReLU` for ReLU, and `nn.Dropout` for Dropout. 
