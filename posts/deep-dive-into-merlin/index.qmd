...
The three flags `ImageEmbedding`, `PhenotypeCls`, `FiveYearPred` select one of Merlin's evaluation/inference modes from the paper:
...
- The default mode — all `False` — gives you the full multi-task pretraining output. 
...
To run [`demo.py`](https://github.com/StanfordMIMI/Merlin/blob/main/documentation/demo.py), we borrow a `gpu_1x_a100_sxm4` instance (1 A100 GPU with 40GB of VRAM) from Lambda Cloud, which currently costs $1.99/hour. The model loads and runs on this instance without any out-of-memory errors, demonstrating that Merlin can be trained and run on a single GPU.
