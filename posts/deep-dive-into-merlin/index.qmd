...
This post is a code-first deep dive into a foundation model called Merlin that leverages the structured and unstructured EHR within hospitals to train an abdominal CT visual model. The code repository for Merlin is on [GitHub](https://github.com/StanfordMIMI/Merlin) and the accompanying Nature paper is [here](https://www.nature.com/articles/s41586-026-10181-8).

Merlin is evaluated on non-adapted and adapted tasks. The non-adapted tasks include zero-shot classification (31 classification tasks), phenotype classification (692 classification tasks), and zero-shot cross-modal retrieval (image to findings and image to impressions). The adapted tasks include 5-year disease prediction (6 classification tasks), radiology report generation, and 3D segmentation (20 organs). The authors report that training Merlin required approximately 160 hours on a single NVIDIA A6000 GPU, making it accessible for researchers with limited computational resources.


### MerlinArchitecture

We dive into the [code](https://github.com/StanfordMIMI/Merlin/blob/main/merlin/models/build.py) for the `MerlinArchitecture` class, which is the core architecture of the Merlin model. The `MerlinArchitecture` class uses two sub-classes, TextEncoder and ImageEncoder:
...
The init method initializes the three mutually-exclusive mode flags (`ImageEmbedding`, `PhenotypeCls`, `FiveYearPred`) and creates instances of the `TextEncoder` and `ImageEncoder` with the appropriate flags. Also, it creates a temperature parameter for contrastive learning.
...
This section demonstrates how to use the `Merlin` model to obtain phenotype predictions from the input images. It finds the top 3 predicted phenotypes from the model's output and prints them in a readable format:
...
(In report_generation_demo.py)

This section demonstrates usage of the Merlin model for radiology report generation. It initializes the `Merlin` model with the `report_generation` configuration, which uses the `Clip3DForTextGeneration` architecture (a vision-to-text model that generates radiology reports from 3D medical images). We pass the images through the model to generate radiology reports. The generated reports are printed in a readable format, allowing us to see how well the model can produce coherent and relevant radiology reports based on the input CT images.
